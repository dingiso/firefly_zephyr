...
void Buzzer::Beep(uint8_t volume, uint16_t frequency_hz, uint16_t duration_ms) {
  ...
  pwm_pin_set_usec(device_, DT_PWMS_CHANNEL(DT_ALIAS(buzzer)), cycle_period_us, pulse_width_us, /*flags=*/0u);
  t_.RunDelayed(duration_ms);
}

void Buzzer::Silence() {
  pwm_pin_set_usec(device_, DT_PWMS_CHANNEL(DT_ALIAS(buzzer)), 1000, 0, /*flags=*/0u);
}